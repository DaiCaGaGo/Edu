...
namespace Form_South_Send_Sms
{
    public partial class SouthSendSms : Form
    {
        #region Variables

        private BackgroundWorker _bgSendSms;
        private readonly ILog Log = LogManager.GetLogger("LogSouthSendSms");
        ...
        private void _bgSendSms_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {                
                using (var connection = factory.CreateConnection())
                {
                    ...
                    {
                        channel.QueueDeclare(queue: "south_task_queue",
                                             durable: true,
                                             exclusive: false,
                                             autoDelete: false,
                                             arguments: null);
                        ...
                        while (true)
                        {
                            var data = channel.BasicGet("south_task_queue", true);
                            if (data == null)
                                break;
                            var body = data.Body;
                            ...
                            {
                                //SetListBox("SMSID " + sms.SmsId.ToString() + Environment.NewLine);
                                ...
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            //int result = SouthApi.SendSMS(ConfigurationManager.AppSettings["username"],
                                            ...
                                            string url = ConfigurationManager.AppSettings["urlRequestSouth"];

                                            string pass = ConfigurationManager.AppSettings["passwordSouth"];
                                            string statusCode = "", responseMess = "";
                                            GetResponseFromUrl(sms.SmsId.ToString(), senderName, sms.Phone, sms.Sms,
                                                ConfigurationManager.AppSettings["usernameSouth"],
                                                pass,
                                            ...
                                            code = GetStringResultFromCode(int.Parse(table["status"].ToString()));
           ...
           string authuser, string authpass, string url, ref string statusCode, ref string responseMess)
        {
            try
            {
                ...
                request.Headers.Add("Authorization", "Basic b25lc21zOnphR0U1enlQREI=");
                ...
                using (var streamWriter = new StreamWriter(request.GetRequestStream()))
                {
                    ...
                    values.Add("from", brandname);
                    values.Add("to", msisdn);
