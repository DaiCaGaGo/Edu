...
namespace Form_Other_Send_Sms
{
    public partial class Main : Form
    {
        ...
        private static readonly ILog Log = LogManager.GetLogger("LogOtherSendSms");
        ...
        private void _bgSendSms_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {                
                using (var connection = factory.CreateConnection())
                {
                    ...
                    {
                        channel.QueueDeclare(queue: "other_task_queue",
                                             durable: true,
                                             exclusive: false,
                                             autoDelete: false,
                                             arguments: null);
                        ...
                        while (true)
                        {
                            var data = channel.BasicGet("other_task_queue", true);
                            if (data == null)
                                break;
                            var body = data.Body;
